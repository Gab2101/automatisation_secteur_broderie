...
import { X, User, Globe, Tag, AlertCircle, Save } from 'lucide-react';
import { Operator, DescriptionTag } from '../types';

interface EditOperatorModalProps {
  isOpen: boolean;
  onClose: () => void;
  operator: Operator | null;
  onUpdateOperator: (operatorId: string, operatorData: {
    name: string;
    language: string;
...
const EditOperatorModal: React.FC<EditOperatorModalProps> = ({
  ...
  const [selectedStrengths, setSelectedStrengths] = useState<DescriptionTag[]>([]);
  ...
  useEffect(() => {
    if (operator) {
      ...
      setSelectedStrengths(operator.strengths || []);
    }
  }, [operator]);
  ...
  const toggleStrength = (tag: DescriptionTag) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Le nom de l\'opérateur est requis';
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Modifier l'Opérateur</h2>
            <p className="text-sm text-gray-600 mt-1">
              Modifier les informations de l'opérateur
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Sélectionnez les domaines d'expertise de cet opérateur
              </p>
              <div className="space-y-4">
                {/* Vignettes disponibles */}
                <div className="flex flex-wrap gap-2">
                  {allDescriptionTags.map((tag) => {
                    const isSelected = selectedStrengths.some(t => t.id === tag.id);
                    return (
                      <button
                        key={tag.id}
                        type="button"
                ...
                {selectedStrengths.length > 0 && (
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <p className="text-xs text-gray-600 mb-2">Points forts sélectionnés :</p>
                    <div className="flex flex-wrap gap-1">
                      {selectedStrengths.map((tag) => (
                        <span
                          key={tag.id}
